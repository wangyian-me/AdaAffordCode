...
parser.add_argument('--primact_type', type=str, default='pushing')
...
def run_collect(cnt_id=0, trial_id=0, primact_type=args.primact_type,
    ...
    if args.test:
        ...
        out_dir = os.path.join(out_dir, '%s_push_%s' % (args.date, args.collect_num))
    ...
    while trial_id < 4:
        ...
        for i in range(900):
            ...
            if success:
                ...
                tot_motion = out_info['joint_angles_upper'][target_part_joint_idx] - out_info['joint_angles_lower'][
                    target_part_joint_idx] + 1e-8
                gt_motion = abs(out_info['final_target_part_qpos'] - out_info['start_target_part_qpos'])

                if gt_motion > args.true_thres or gt_motion / tot_motion > 0.5:
                    out_info['gt_labels'] = 1
                    with open(os.path.join(out_dir, 'result_%d_%d.json' % (trial_id, succ_cnt)), 'w') as fout:
                        json.dump(out_info, fout)
                elif (succ_cnt > 10 or i > 200): # we need to make sure there is enough success data if it is possible
                    out_info['gt_labels'] = 0
                    with open(os.path.join(out_dir, 'result_%d_%d.json' % (trial_id, succ_cnt)), 'w') as fout:
                        json.dump(out_info, fout)
                else:
