...
class Affordance(nn.Module):
    def __init__(self, feat_dim, hidden_dim=32):
        ...
        self.mlp3 = nn.Linear(1, hidden_dim)
        self.mlp1 = nn.Linear(feat_dim + hidden_dim + hidden_dim, feat_dim)
        self.mlp2 = nn.Linear(feat_dim, 1)
        self.BCELoss = nn.BCEWithLogitsLoss(reduction='none')
        self.L1Loss = nn.L1Loss(reduction='none')


    def forward(self, pcs, hidden_info, aff_scores):
        # pcs[:, 0] = contact_point
        pcs = pcs.repeat(1, 1, 2)
        whole_feats = self.pointnet2(pcs)

        net = whole_feats[:, :, 0]
        aff_scores = aff_scores.view(-1,1)
        y = torch.cat([aff_scores], dim=-1)
        y = self.mlp3(y)
        net = torch.cat([net, hidden_info, y], dim=-1)
        net = self.mlp1(net)
        net = F.leaky_relu(net)
        net = self.mlp2(net).squeeze(-1)
        return net

    def inference_whole_pc(self, input_pcs, hidden_info, aff_scores):
        ...
        aff_scores = aff_scores.reshape(batch_size * pt_size, 1)
        y = torch.cat([aff_scores], dim=-1)
        y = self.mlp3(y)
        x = torch.cat([net, hidden_info, y], dim=-1)
        x = self.mlp1(x)
        x = F.leaky_relu(x)
        pred_result_logits = self.mlp2(x).squeeze(-1)
        return pred_result_logits

    def get_loss(self, pcs, hidden_info, gt_score, aff_scores):
        score = self.forward(pcs, hidden_info, aff_scores)
