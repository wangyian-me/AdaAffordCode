...
class Network(nn.Module):
    ...
    def forward(self, pcs, dirs1, dirs2, gt_result):
        ...
        with torch.no_grad():
            ...
            # print(avg_proposal_succ_scores)
        ...
        return critic_loss_per_data, action_score_loss_per_data
    ...
    def inference_actor_whole_pc(self, pcs):
        pcs = pcs.repeat(1, 1, 2)
        batch_size = pcs.shape[0]
        pt_size = pcs.shape[1]
        whole_feats = self.pointnet2(pcs)
        ...
        rvs = torch.randn(batch_size * pt_size, 1, self.rv_dim).float().to(net.device)
        expanded_net = net.reshape(batch_size * pt_size, -1)
        expanded_rvs = rvs.reshape(batch_size * pt_size, -1)
        expanded_pred_s6d = self.actor(expanded_net, expanded_rvs)
        pred_s6d = expanded_pred_s6d.reshape(batch_size * pt_size, 6)
        return pred_s6d

    def inference_nactor_whole_pc(self, pcs, n):
        pcs = pcs.repeat(1, 1, 2)
        batch_size = pcs.shape[0]
        pt_size = pcs.shape[1]
        whole_feats = self.pointnet2(pcs)
