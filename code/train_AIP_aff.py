...
def train(conf, train_data_list, val_data_list):
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        for train_batch_ind, batch in train_batches:
            ...
            losses = aff_forward(batch=batch, data_features=data_features, network=network,  critic = critic, actor_network = actor, AIP=AIP_critic, conf=conf, is_val=False, \
            ...
            while val_fraction_done <= train_fraction_done and val_batch_ind + 1 < val_num_batch:
                ...
                with torch.no_grad():
                    # forward pass (including logging)
                    total_loss = aff_forward(batch=val_batch, data_features=data_features, network=network, critic = critic, actor_network = actor, AIP=AIP_critic,
...
def aff_forward(batch, data_features, network, critic, actor_network, AIP, conf,
                   is_val=False, step=None, epoch=None, batch_ind=0, num_batch=1, start_time=0,
                   log_console=False, log_tb=False, tb_writer=None, lr=None):

    global tot_rand, tot_l, tot_r
    ...
    idx2 = random.sample(range(32), length)
    # idx2 = [0]
    with torch.no_grad():
        hidden_info = critic.hidden_encoder(dir[idx2], dis[idx2], push_dis[idx2], ctpt[idx2], joint_info[idx2],
                                          ...
                                          f_dir[idx2])
        hidden_info = hidden_info.repeat(batch_size, 1).view(batch_size, -1).to(conf.device)
        # thre = 0
        # while thre < 50:
        #     thre += 1
        ...
        aip_scores = AIP.inference_naction(up, input_pcs, forward, hidden_info, action_scores, actor_network.rv_cnt)
        aip_scores = aip_scores.view(batch_size, actor_network.rv_cnt)
        gt_scores = aip_scores.view(batch_size, actor_network.rv_cnt, 1).topk(k=5, dim=1)[0].mean(dim=1).view(-1)
        action_scores = action_scores.view(batch_size, actor_network.rv_cnt)
        action_scores = action_scores.view(batch_size, actor_network.rv_cnt, 1).topk(k=5, dim=1)[0].mean(dim=1).view(-1)
            # print(gt_scores)
            # flag = False
            # for i in range(batch_size):
            #     if gt_scores[i].item() > 0.1:
            #         flag = True
            #         break
            # if flag:
            #     break
        # for i in range(32):
        #     if i >= 16:
        #         tot_rand += gt_scores[i].item()
        #     elif i%2 == 0:
        #         tot_r += gt_scores[i].item()
        #     else :
        #         tot_l += gt_scores[i].item()
        # print("gt_scores", gt_scores)
    # print("tot_rand:",tot_rand/2,"tot_l",tot_l,"tot_r",tot_r)
    loss = network.get_loss(input_pcs, hidden_info, gt_scores + action_scores, action_scores)
